...
\begin{defin}
Seja $A$ uma matriz quadrada de ordem $n$. Dizemos que um vetor {\color{red}não nulo} {\color{blue}$\vec{v}\in \R^n$} é um  {\color{blue}autovetor} de $A$ se $A\vec{v}$ for um múltiplo escalar de $\vec{v}$, isto é, se existe  {\color{orange}$\lambda \in \R$} tal que 
\[A{\color{blue}\vec{v}}={\color{orange}\lambda} {\color{blue}\vec{v}}.\]
Neste caso, dizemos que {\color{orange}$\lambda \in \R$} é um {\color{orange}autovalor} de $A$.
...
\begin{teo}
{\color{orange}$\lambda \in \R$} é um autovalor de uma matriz $A$ se, e só se, 
\[\det(A-{\color{orange}\lambda}I)=0.\]
Está equação e dita {\color{blue}equação característica de A}. E o polinômio $p({\color{orange}\lambda})=\det(A-{\color{orange}\lambda} I)$ é denominado {\color{blue}polinômio característico}.
\end{teo}
...
Os autovetores associados a um mesmo autovalor {\color{orange}$\lambda$} são os vetores não nulos  que satisfazem
\[(A-{\color{orange}\lambda}  I)\vec{X}=\vec{0},\]
...
A=sp.Matrix([[0,0,-2], [1,2,1], [1,0,3]])
[P,D]=A.diagonalize()
\end{pycode}

\begin{exe}
Defina $P=\pyl{P}$, a matriz cujas colunas são os autovetores da matriz 
$A=\pyl{A} $ do exemplo anterior e calcule a matriz  $D=P^{-1}AP$, semelhante a $A$.
\end{exe}
...
\begin{teo}
Uma matriz $A$ de ordem $n$ é {\color{blue} diagonalizável} se, e só se, {\color{red}possui $n$ autovetores linearmente independentes}. 

Neste caso,  se ${\color{blue}D}={\color{red}P}^{-1}A{\color{red}P}$, então a  matriz diagonal {\color{blue}$D$ é formada pelos autovalores} e a matriz {\color{red}$P$ tem as colunas formadas pelos autovetores}.
\end{teo}
