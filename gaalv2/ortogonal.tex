...
As colunas de uma matriz quadrada \(Q\) formam um conjunto {\color{blue}ortonormal} se, e somente se \(Q^{-1}=Q^t\). A esse tipo de matriz, damos o nome de \dt{matriz ortogonal}.

\begin{exe}
A seguinte matriz é ortogonal
\[
\begin{bmatrix}
\frac{1}{3} & \frac{2}{3} & \frac{2}{3} \\[0.8em]
\frac{2}{3} & -\frac{2}{3} & \frac{1}{3} \\[0.8em]
...
Uma classe especial de matrizes ortogonais são as chamadas \dt{Matrizes de Rotação}. A seguinte matriz gira um {\color{blue}vetor do plano} por um ângulo $\theta$ no sentido anti-horário.
\[R_\theta= 
\begin{bmatrix}
\cos(\theta) & -\sen(\theta)\\
\sen(\theta) & \cos(\theta)
\end{bmatrix}
\]
A seguinte matriz gira um {\color{blue}vetor do espaço} por um ângulo $\theta$ em torno do eixo \(z\).
\[R_\theta= 
\begin{bmatrix}
\cos(\theta) & -\sen(\theta)& 0\\
\sen(\theta) & \cos(\theta) & 0\\
...
\begin{frame}[label=orto]{Teorema Espectral para Matrizes Simétricas}
%\begin{prop}
%Se $A$ é uma matriz simétrica, então os autovetores associados a autovalores distintos são ortogonais.
%\end{prop}
Dizemos que uma matriz quadrada $A$ é \dt{ortogonalmente diagonalizável} se é 
diagonalizável e a matriz $Q$ que a diagonaliza é ortogonal.

\begin{block}{Teorema Espectral}
Uma matriz  $A$ quadrada com entradas reais. Então
\begin{center}
 $A$ é {\color{blue}ortogonalmente diagonalizável} \(\Leftrightarrow\) $A$ é {\color{blue} simétrica}.
\end{center}
Além disso, 
\begin{center}
{\color{red}autovetores associados a autovalores distintos são ortogonais}.
\end{center}
\end{block}
...
\begin{casa}
Encontre a matriz \(Q\) que diagonaliza a matriz
\[
A=\begin{bmatrix}
4 & 2 & 2 \\
2 & 4 & 2 \\
