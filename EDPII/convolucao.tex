...
\begin{frame}
	\begin{defin}\label{def-conv}
		...
		\[(f\ast g)(x)=\int_{\R^\dm}f(x-y)g(y)\,dy.\]
	\end{defin}
	\begin{prop}
...
\begin{frame}{Derivação da convolução}
	...
	\begin{prop}
		Sejam  $f\in C_c^k(\R^\dm)$ $(k\geq 1)$ e $g\in \Lloc^1(\R^\dm)$. Então $f\ast g\in C^k(\R^\dm)$ e 
		\[D^\alpha(f\ast g)=(D^\alpha f)\ast g,\ \forall \alpha\in \N \text{ com } |\alpha|\leq k.\]
