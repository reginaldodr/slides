...
\begin{defin}
 Sejam $\Omega \subset \R^\dm$, $k\geq 0$ um inteiro e $1\leq p\leq +\infty$. O \dt{Espaço de Sobolev} $W^{k,p}(\Omega)$  é o espaço vetorial normado definido por 
 \[W^{k,p}(\Omega):=\{u\in L^p(\Omega);\ D^\alpha u\in L^p(\Omega),\ \forall\, |\alpha|\leq k\},\]
 com a norma dada por 
 \[\n{u}_{k,p,\Omega}=\n{u}_{W^{k,p}(\Omega)}=\left(\sum_{|\alpha|\leq k} \n{D^\alpha u}^p_{L^p(\Omega)}\right)^{1/p}.\]
