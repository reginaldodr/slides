...
Dados $\Omega\subset \R^\dm$ um aberto e {\color{red}$\ep>0$}, defina {\color{red}$\Omega_\ep=\{x\in \Omega;\ \operatorname{dist}(x,\partial \Omega)>\ep\}$}. Dado $u\in L^p(\Omega)$ denotemos por $\tilde{u}$ a extensão a zero de $u$. Se $\rho\in C^\infty_c(\R^n)$, defina
...
\begin{teo}[Friedrichs]
	Sejam $\Omega \subset \R^\dm$ um aberto e $u\in W^{k,p}(\Omega)$ com $1\leq p<+\infty$. Então, existe uma sequência $(v_n)$ em $\cc(\rd)$ tal que:
	\begin{equation*}
		\begin{split}
...
\begin{frame}{Regra da Cadeia}
	\begin{prop}[Regra da Cadeia]
		Seja $G\in C^1(\R)$ tal que $G'\in L^{\infty}(\R)$ e \(G(0)=0\). Se $u\in W^{1,p}(\Omega)$, então $G\circ u\in W^{1,p}(\Omega)$ e 
...
\includegraphics[scale=0.25]{images/meyers-serrin.png}
\end{minipage}
