...
Dado $u:Q_+\longrightarrow \R$, denote $u^\ast:Q\longrightarrow$ \dt{a extensão  por reflexão} de $u$, isto é,
\[u^\ast(x',x^\dm)=\begin{cases}
	u(x',x^\dm), & x^\dm>0,\\ u(x',-x^\dm), & x^\dm<0.
\end{cases}\]
	
	\begin{lema}
		Seja $u\in W^{1,p}(Q_+)$ (ou $ W^{1,p}(\R^\dm_+)$), com $1\leq p\leq +\infty$. Então $u^\ast\in W^{1,p}(Q)$ (ou $ W^{1,p}(\R^\dm)$) e 
		\[\n{u^\ast}_{p,Q}\leq 2\n{u}_{p,Q_+} \text{ e } \n{u^\ast}{1,p,Q}\leq 2\n{u}_{1,p,Q_+}.\]
