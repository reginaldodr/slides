...
\begin{obs}
Cabe ressaltar que, por {\color{blue} \(\varphi\)} ser um difeomorfismo, sempre podemos trocar {\color{blue} \(Q_r\)} por \(Q_1\).
\end{obs}
...
\begin{frame}
\begin{block}{Notação}
Vamos denotar por \(\cc(\overline{\Omega})\) o conjunto de todas as restrições a \(\overline{\Omega}\) das funções em \(\cc(\rd)\), isto é, 
\[
\cc(\overline{\Omega})=\left\{ \varphi\big\vert_{\overline{\Omega}};\ \varphi\in \cc(\rd)  \right\}.
\] 
\end{block}


	\begin{corol}
Seja \(\Omega\subset \rd \) um aberto {\color{red} com fronteira de classe \(C^1\) limitada}. Então
\[
\overline{\cc(\overline{\Omega})}^{\|\cdot\|_{1,p,\Omega}}=W^{1,p}(\Omega),
\]
% \(\cc(\overline{\Omega})\)é denso de \( W^{1,p}(\Omega)\)
com \(1\leq p<\infty\). Precisamente, se \(u\in W^{1,p}(\Omega)\), então existe \((u_n)_{n\in \N}\subset C^\infty_c(\R^\dm)\) tal que 
