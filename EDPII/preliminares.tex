...
\begin{frame}{Teorema de Mundança de Variáveis}
%	
\begin{block}{Teorema de Mudança de Variáveis}
Sejam \(\Omega\subset \rd\) um conjunto aberto e {\color{blue}\(G:\Omega \longrightarrow \rd\)} um {\color{blue}difeomorfismo de classe \(C^1\)}. Se {\color{blue}\(f\geq 0\)} ou \(f\in L^1({\color{blue}G(\Omega)})\), então
\[
\int_{{\color{blue}G(\Omega)}}f({\color{blue}y})\,{\color{blue}dy}=\int_{\color{red}\Omega} f\circ G({\color{red}x})|\det DG({\color{red}x})|\,{\color{red}dx}.
\]
