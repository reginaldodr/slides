...
\end{enumerate}
\end{defin}
\begin{exe} Decida se os conjuntos abaixo são subespaços.
\begin{enumerate}
\item $V=\{(x,y,z)\in \R^3;\ z=0\}$.
\item \(W=\{(x,y)\in \R^2;\ x+y=1\}\}\).
\item \(X=\{(x,y,z)\in \R^3;\ y=0 \text{ e } z=x^2\}\}\) 
\item \(Z=\{(x,y)\in \R^2;\ y\geq 0  \} \)
\end{enumerate}
\end{exe}
...
\item O vetor nulo pertence a qualquer subespaço.
...
\item Retas, passando pela origem, são todos os subespaços não triviais de $\R^2$.

\item Retas e planos em $\R^3$, passando pela origem, são todos os subespaços não triviais de $\R^3$.
...
v1=sp.Matrix([1,0,1,2,1])
v2=sp.Matrix([1,2,0,-1,0])
v3=sp.Matrix([-3,2,0,1,1])

v4=3*v1-v2
v5=v1-v2+v3
A=sp.Matrix.hstack(v1,v2,v3,v4,v5)
\end{pycode} 
\begin{exe}
 
\begin{enumerate}
\item Determine uma base e a dimensão para o subespaços
\[V=\{(a+c,b+c,a+b+2c);\, a,b,c\in \R\}.\]
\item Seja
\[W=\operatorname{span}\left\{ \pyl{v1},\pyl{v2},\pyl{v3},\pyl{v4},\pyl{v5} \right\}.\]
Obtenha uma base para \(W\) dentre seus geradores.
\end{enumerate} 
\end{exe}
\end{frame}
%------------------------------------------
...
%Este último exemplo motiva a seguinte definição. Dada uma matriz $A$  $m\times n$. {\color{blue}O espaço coluna de $A$} é o  subespaço de $\R^m$ gerados pelas colunas de $A$.
%\medskip
Dadas duas matrizes ${\color{blue}A}$ e ${\color{red}R}$ linha-equivalentes, sabemos que:
\begin{enumerate}
\item  Os sistemas ${\color{blue}A}X=\vec{0}$ e ${\color{red}R}X=\vec{0}$ {\color{red}têm exatamente as mesmas soluções!}
\item Uma solução $X=\begin{bmatrix}
\alpha_1\\ \alpha_2 \\ \vdots \\ \alpha_n
\end{bmatrix}$ desses sistemas nos dá uma combinação linear das colunas, isto é, se {\color{blue}$A_1, A_2, \ldots, A_n$} e {\color{red}$R_1, R_2,\ldots R_n$} são as respectivas colunas de {\color{blue}$A$} e {\color{red}$R$}, então
\[\alpha_1{\color{blue}A_1}+\alpha_2{\color{blue}A_2}+\cdots \alpha_n{\color{blue}A_n}=\vec{0}\]
\[\alpha_1{\color{red}R_1}+\alpha_2{\color{red}R_2}+\cdots \alpha_n{\color{red}R_n}=\vec{0}\]
\end{enumerate}
Em outras palavras,  ao escalonarmos uma matriz, {\color{red} as colunas mudam mas a dependência linear permanece!}
...
\begin{exampleblock}{}
Em particular, os espaços linha e coluna de $R$ têm a mesma dimensão, apesar de serem  subespaços de espaços vetoriais diferentes!
\end{exampleblock}
...
Contudo, como vimos anteriormente,  o escalonamento {\color{red} mantém a dependência linear} das colunas. Portanto, para obtermos uma base do espaço coluna de uma matriz \(A\), devemos:
\begin{enumerate}
\item Escalonar a matriz \(A\);
\item Selecionar os vetores colunas de \(A\) que correspondem aos pivôs da matriz escalonada.
\end{enumerate}
\end{frame}
