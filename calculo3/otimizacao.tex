...
\begin{frame}[label=otimizacao]{Problemas com restrições}

\begin{block}{ }
Uma companhia possui três fábricas produzindo o mesmo produto. Sabendo-se que:
\begin{itemize}
\item A fábrica {\color{blue} \(A\)} produz {\color{blue} \(x\)} unidades a um custo de \(3x^2+200\);
\item A fábrica {\color{red} \(B\)} produz {\color{red} \(y\)} unidades a um custo de \(y^2+400\);
\item A fábrica {\color{orange} \(C\)} produz {\color{orange} \(z\)} unidades a um custo de \(2z^2+300\);
\end{itemize}
se um pedido de 1.100 unidades deve ser entregue, como a produção deve ser distribuída entre as três fábricas a fim de minimizar os custos?
\end{block}

\[
\begin{cases}
\min (3{\color{blue}x}^2+{\color{red}y}^2+2{\color{orange}z}^2+900)\\
\text{restrição: } {\color{blue}x}+{\color{red}y}+{\color{orange}z}=1.100
\end{cases}
\]
...
\begin{center}
\includegraphics[scale=.2]{figuras/otimizacao/lagrange1.png}
%sec14_8-fig1.png}
\end{center}
...
\begin{frame}[label=otimizacao]
\begin{exe}
Determine as coordenadas dos pontos do círculo $x^2+y^2=1$  cujo produto entre eles é o maior possível.
\end{exe}
...
\begin{frame}[label=otimizacao]
\frametitle{Multiplicadores de Lagrange no espaço}
%\begin{scriptsize}
...
\includegraphics[scale=0.3]{figuras/otimizacao/lagrange3d.png}
...
\begin{frame}[label=otimizacao]
\begin{exe}
Considere uma placa circular  \(D=\{(x,y)\in \R^2; 
x^2+y^2\leq 1 \}\). Sabendo-se que a temperatura em graus na placa é dada por  \(T(x,y)=x^2+2y^2\), determine os pontos mais frio e mais quente.
\end{exe}
\begin{center}

\includegraphics[scale=0.4]{figuras/otimizacao/lagrange2.png}
